...
```ts TypeScript
const res = await fetch("https://sentvia-api-production.up.railway.app/v1/inboxes", {
  method: "POST",
...
import requests
inbox = requests.post(
    "https://sentvia-api-production.up.railway.app/v1/inboxes",
...
```ts TypeScript
await fetch("https://sentvia-api-production.up.railway.app/v1/messages", {
  method: "POST",
  headers: { Authorization: `Bearer ${process.env.SENTVIA_KEY}`, "Content-Type": "application/json" },
  body: JSON.stringify({
    inbox_id: inbox.id,
